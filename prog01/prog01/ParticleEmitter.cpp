...
void ParticleEmitter::Add()
{
	for (int i = 0; i < 10; i++)
	{
		...
		position.x = (float)rand() / RAND_MAX * md_pos - md_pos / 2.0f;
		position.y = (float)rand() / RAND_MAX * md_pos - md_pos / 2.0f;
		position.z = (float)rand() / RAND_MAX * md_pos - md_pos / 2.0f;
		...
		//追加
		particleMan->Add(60, position, velocity, accel, s_scale, e_scale, s_color, e_color);
	}
}
